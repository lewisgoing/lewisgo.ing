...
import { HiForward, HiBackward, HiQueueList } from "react-icons/hi2";
...
const AudioBox = () => {
  ...
  return (
    <>
      ...
      >
        {/* Top Bar */}
        <div className="absolute left-0 top-0 z-[1] w-14 h-14 flex items-center justify-center m-3 rounded-full bg-tertiary/50">
          <HiQueueList size={40} className={"text-primary p-1"} />
        </div>
        ...
        >
          ...
          >
            ...
            <div
              className="text-sm h-full w-full px-2 rounded-lg leading-snug mt-1"
              // style={{ border: "1px solid red" }}
            >
<div style={progressBarStyle} onClick={handleProgressBarClick}>
  <div style={progressIndicatorStyle((currentTime / duration) * 100)}></div>
  <div style={dotIndicatorStyle(progressPercentage)}></div>
   </div>
              <div className="flex flex-row pt-1">
                <div
                  className="text-[10px] text-left w-1/2"
                  // style={{ border: "1px solid red" }}
                >
                  {formatTime(currentTime)}
                </div>
                <div
                  className="text-[10px] text-muted-foreground text-right w-1/2"
                >
                  {formatTime(duration)}
                </div>
              </div>
          ...
          {/* Buttons */}
          <div
            className="flex flex-row w-full h-fit rounded-lg items-center justify-between bg-tertiary/50"
            // style={{ border: "1px red solid" }}
          >
            {/* Back */}
            <div
